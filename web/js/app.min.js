...
;null==c&&window.console&&console.error&&console.error("The select2('"+b+"') method was called on an element that is not using Select2."),d=c[b].apply(c,f)}),a.inArray(b,e)>-1?this:d}throw new Error("Invalid arguments for Select2: "+b)}}return null==a.fn.select2.defaults&&(a.fn.select2.defaults=d),c}),{define:b.define,require:b.require}}(),c=b.require("jquery.select2");return a.fn.select2.amd=b,c}),top!=self&&(top.location.href=location.href),$(document).ready(function(){$('[data-toggle="auto-dismiss"]').hide(),$('[data-toggle="auto-dismiss"]').fadeIn("low"),$('[data-toggle="auto-dismiss"]').delay("5000").fadeOut("low")}),$(document).ready(function(){var $cartBadge=$("a#cart span.badge");$("a.cart-add-btn").click(function(e){e.preventDefault();var $url=$(this).attr("href");$.get($url,function(data){var $nbItems=data.items.length;$cartBadge.text($nbItems),!0===data.reached_limit&&location.reload()})}),$("a.cart-remove-btn").click(function(e){e.preventDefault();var $url=$(this).attr("href"),$tableRow=$(this).closest("tr");$.get($url,function(data){var $nbItems=data.items.length;$cartBadge.text($nbItems),$tableRow.remove()})})});var delay=function(){var timer=0;return function(callback,ms){clearTimeout(timer),timer=setTimeout(callback,ms)}}();
